...
public class WebSecurityConfiguration implements SecurityConfiguration{
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(request -> request.requestMatchers("/api/v1/auth/**")
                        ...
                        .requestMatchers("/api/v1/agent/**").hasAnyAuthority(UserRole.AGENT.name())
                        .requestMatchers("/api/v1/user/**").hasAnyAuthority(UserRole.CUSTOMER.name())
